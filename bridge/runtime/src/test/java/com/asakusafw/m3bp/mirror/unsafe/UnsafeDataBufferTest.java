...
/**
 * Test for {@link UnsafeDataBuffer}.
 */
public class UnsafeDataBufferTest {
    ...
    private static final class Buf extends UnsafeDataBuffer {

        private final Memory memory;

        public Buf(long size) {
            this.memory = new Memory(size);
            rewind();
        }

        public void rewind() {
            this.dataPtr = Pointer.nativeValue(memory);
        }

        public int offset() {
            return (int) (dataPtr - Pointer.nativeValue(memory));
        }
    }
}
