...
/**
 * Executes M3BP tasks on the testing process.
 */
public class InProcessM3bpTaskExecutor implements TaskExecutor {
     ...
     * @return this
     */
    public InProcessM3bpTaskExecutor withEngine(String key, String value) {
    ...
    public boolean isSupported(Context context, TaskReference task) {
        return task instanceof CommandTaskReference
                && task.getModuleName().equals(M3bpTask.MODULE_NAME);
    }
    ...
    public void execute(Context context, TaskReference task) throws InterruptedException, IOException {
        ...
        try (URLClassLoader loader = getLaunchClassLoader(context, jobflow)) {
            try {
                ...
                throw new IOException(MessageFormat.format(
                        "error occurred while launching M3BP: args={0}",
                        arguments));
            }
        }
    }
    ...
    private List<String> getLaunchArguments(Context context, CommandTaskReference command) {
        ...
        File engineConf = TaskExecutors.getFrameworkFile(context, M3bpTask.PATH_ENGINE_CONFIG);
